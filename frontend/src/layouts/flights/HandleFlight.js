...
function HandleFlight({ operation, flightToUpadte }) {
  ...
  const [selectedCateringCompanies, setSelectedCateringCompanies] = useState(
    []
  ); // "cateringCompanies": [{"id": 1}, ...
  ...
  useEffect(() => {
    if (operation === "UPDATE") {
      setFormValues({
        ...flightToUpadte,
        flightCrew: flightToUpadte.flight_crew,
      });
    }
  ...
  const handleCateringCompantSelection = (e, cateringCompany) => {
    if (verifySelectedCateringComapny(cateringCompany)) {
      let filtredArray = selectedCateringCompanies.filter(
        (item) => item.id !== cateringCompany.id
      );
      setSelectedCateringCompanies(filtredArray);
    } else {
      setSelectedCateringCompanies([
        ...selectedCateringCompanies,
        cateringCompany,
      ]);
    }
  };

  const displayCateringCompanies = cateringCompanies.map(
    (cateringCompany, index) => (
      <FormControlLabel
        control={
          <Checkbox
            name="vegetarian"
            ...
            onChange={(e) => {
              handleCateringCompantSelection(e, cateringCompany);
            }}
      ...
      if (tmpSelectedCateringCompanies[i]["id"] === cateringCompany.id) {
        for (let j = 0; j < cateringCompany.menus.length; j++) {
          if (cateringCompany.menus[j]["id"] === menuId) {
            if (
              tmpSelectedCateringCompanies[i]["menus"][j]["selected"] ==
              undefined
            ) {
  ...
  const displayMenus = selectedCateringCompanies.map(
    (cateringCompany, index) => (
      <div key={index}>
          ...
          <FormControlLabel
            control={
              <Checkbox
                ...
                onChange={(e) => {
                  handleMenuSelection(e, cateringCompany, menu.id);
                }}
  ...
  const handleCrewAssignment = (e, crewMember) => {
    if (verifyCrewMember(crewMember)) {
      ...
      let filtredArray = formValues.flightCrew.filter(
        (item) => item.id !== crewMember.id
      );
      ...
      setFormValues({
        ...formValues,
        flightCrew: [...formValues.flightCrew, crewMember],
      });
    }
  };
  ...
  const displayPilots = pilots.map((pilot, index) => (
    <FormControlLabel
      key={index}
      checked={pilot.id === formValues.pilot.id}
      value={pilot.id}
      control={<Radio />}
  ...
  const displayCoPilots = coPilots.map((coPilot, index) => (
    <FormControlLabel
      key={index}
      checked={coPilot.id === formValues.coPilot.id}
      value={coPilot.id}
      control={<Radio />}
      label={`${coPilot.firstname} ${coPilot.lastname}`}
    />
  ));
    ...
    <FormControlLabel
      control={
        <Checkbox
          ...
          onChange={(e) => {
            handleCrewAssignment(e, crewMember);
          }}
          value={crewMember.id}
        />
      }
          ...
          <Accordion>
            ...
            <AccordionDetails>
              ...
              >
                {/* ... */}
                <FormControl>
                  <FormLabel id="pilot-radio-buttons-group-label">
                    Pilot
                  </FormLabel>
                  <RadioGroup
                ...
                <FormControl>
                  <FormLabel id="flight-crew-radio-buttons-group-label">
